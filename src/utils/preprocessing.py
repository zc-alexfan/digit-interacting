...
def load_segm(path, txn):
    # load
    img = sys_utils.read_lmdb_image(txn, path)
    if img is None:
        return img
...
def augmentation(
        img, img_segm, bbox, joint_coord, joint_valid, hand_type, mode,
        joint_type, input_img_shape):
    ...
    img, trans, inv_trans = generate_patch_image(img, bbox, do_flip, scale, rot, input_img_shape, cv2.INTER_LINEAR)
    if img_segm is not None:
        box_segm = copy.deepcopy(bbox)
        img_segm, _, _ = generate_patch_image(img_segm, box_segm, do_flip, scale, rot, input_img_shape, cv2.INTER_NEAREST)
    ...
    return img, img_segm, joint_coord, joint_valid, hand_type, trans, inv_trans, do_flip


def transform_input_to_output_space(
        joint_coord, joint_valid, rel_root_depth, root_valid,
        root_joint_idx, joint_type, input_img_shape, output_hm_shape,
        bbox_3d_size, bbox_3d_size_root, output_root_hm_shape):
    # map the 3d joint in cam space (nonzero root) to the heatmap scale
    # but it does not create the 3d gaussian

    joint_coord = joint_coord.copy(); joint_valid = joint_valid.copy()

    # map xy to heatmap scale
    ...
    # map the relative root depth to heatmap scale
